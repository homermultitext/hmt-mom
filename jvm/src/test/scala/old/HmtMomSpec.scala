...
class HmtMomSpec extends FlatSpec {
  ...
  "An HmtMom" should "create an MID EditorsRepo" in pending /*{
    val mom = HmtMom(repoDir)
    assert(mom.midRepo.baseDir == repoDir)
  }*/

  it should "include a dse directory" in pending /*{
    val mom = HmtMom(repoDir)
    val expectedDse = File(repoDir + "/dse")
    assert (mom.midRepo.dseDir == expectedDse)
  } */

  it should "do MID DSE consistency checking" in pending /* {
    val mom = HmtMom(repoDir)
  }*/


  it should "generate a CITE Library" in pending /*{
    val mom = HmtMom(repoDir)
    val lib = mom.library

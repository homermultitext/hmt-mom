...
* Strings the HmtScholiaString class.
*/
object HmtScholiaOrthography  extends MidOrthography with LogSupport  {
  ...
  */
  def tokenizeNode(n: CitableNode): Vector[MidToken] = {
    ...
    val lgs = HmtScholiaString(n.text)
    ...
    val classified = for ( (unit, idx) <- units.zipWithIndex) yield {
      ...
      val newVersion = urn.addVersion(urn.versionOption.getOrElse("") + HmtScholiaOrthography.exemplarId)
      ...
      if (trimmed.isEmpty) {
        n.text.size match {
          case 0 => {
            warn("HmtScholiaString: empty text citable node " + n)
            Vector.empty[MidToken]
          }
          case _ => {
            warn (s"HmtScholiaString: evil unicode detected.  Text node of length ${n.text.size} has no tokens.")
  ...
  def exemplarId: String = "_hmtschtkn"
  ...
  */
  def lexicalCategory(s: String, punctuationChars: String = punctuationString): Option[MidTokenCategory] = {
    ...
    } else if (HmtScholiaString(s).valid) {
      Some(LexicalToken)
  ...
  * other characters in `ucode` view.*/
  ...
  val validCps = CodePointTranscoder.strToCps(validList)

  def validAsciiCP(cp: Int): Boolean =  validCps.contains(cp) //{
  ...
  */
  def nfcToAscii(ucode: String, ascii: String = "", idx: Int = 0): String = {
    ...
    if (idx >= ucode.length ) {
      ...
      val newAscii = if (validCps.contains(cp)) {
        ascii + CodePointTranscoder.cpsToString(Vector(cp))
      } else {
        ascii + "#" + CodePointTranscoder.cpsToString(Vector(cp)) + "#"
      }
      nfcToAscii(ucode, newAscii, idx + java.lang.Character.charCount(cp))
    }
      /*
